...
place. It grows in documented layers on the go-web-sdk-template baseline it was generated from,
...
The service runs on one declared stack (Postgres for SQL) and uses each capability at the tier
its purpose requires: the library's standard tier by default, and the provider's native
features where they earn it, contained and listed (the README's Stack section). The workspace
roadmap in the coordinator repository, standards-lab, defines version 1.0 and the path to it (goal `v1`). The notes here
carry each layer's design direction:
...
- **Data composition and CQRS** — the organization domain runs on authored SQL; `data-layer.md`
  carries the remaining domains and the CQRS contract.
